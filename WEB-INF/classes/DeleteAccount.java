...
public class DeleteAccount extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException{
		...
		response.setContentType("text/html");
                ...
                out.println("<title>Passmanager Delete Account</title>");
                out.println("</head>");
                out.println("<body>");
		...
		try {
			...
			while (rs.next ()){
				email=rs.getString("AES_DECRYPT(email,'" + session_email + AES_key + "')");
                                ...
                                if(email.equals(session_email) && token.equals(session_token) ){
                                    ...
                                    sql = "DELETE FROM user_data WHERE email = AES_ENCRYPT('" + session_email + "','" + session_email + AES_key + "')";
                                    s.executeUpdate(sql);
                                    
                                    sql = "DELETE FROM users WHERE email = AES_ENCRYPT('" + session_email + "','" + session_email + AES_key + "')";
                                    s.executeUpdate(sql);
			...
			if(delete){
                                ...
                                //response.sendRedirect(response.encodeRedirectURL("/passmanager/main.jsp"));
				response.sendRedirect(response.encodeRedirectURL("/passmanager/login.jsp"));
                                
			}
			else{
                                response.sendRedirect(response.encodeRedirectURL("/passmanager/control_panel.jsp"));
			}
